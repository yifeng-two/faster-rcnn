...
def _process_image(directory, name):
    ...
    for obj in root.findall('object'):
        ...
        # bboxes.append(
        #     (float(bbox.find('ymin').text) / shape[0], float(bbox.find('xmin').text) / shape[1],
        #      float(bbox.find('ymax').text) / shape[0], float(bbox.find('xmax').text) / shape[1]))
        bboxes.append((float(bbox.find('ymin').text), float(bbox.find('xmin').text),
                       float(bbox.find('ymax').text), float(bbox.find('xmax').text)))
...
def _convert_to_example(name, image_data, labels, labels_text, bboxes, shape, difficult, truncated):
    xmin = []
    ymin = []
    xmax = []
    ymax = []
    ...
    example = tf.train.Example(features=tf.train.Features(
        feature={
            ...
            'image/image_id': bytes_feature(name.encode('utf8')),
...
def _add_to_tfrecord(dataset_dir, name, tfrecord_writer):
    ...
    example = _convert_to_example(name, image_data, labels, labels_text, bboxes, shape, difficult,
                                  truncated)
...
def generate_tfrecord(voc_root, year, split, output_dir, shuffling=False):

    output_dir = os.path.join(output_dir, '%s' % split)
    # 如果output_dir不存在则创建
...
if __name__ == '__main__':
    ...
    generate_tfrecord('VOCdevkit', 2007, 'train')
